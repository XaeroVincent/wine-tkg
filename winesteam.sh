...
DXVK_HUD=full WINEESYNC=1 WINE_LARGE_ADDRESS_AWARE=1 WINEDEBUG=-all WINEPREFIX=$HOME/.wine_steam /usr/local/bin/wine "$HOME/.wine_steam/drive_c/Program Files (x86)/Steam/steam.exe" -cef-disable-sandbox -cef-in-process-gpu -cef-disable-gpu-compositing -cef-single-process
